...
func MyFilter(vs []string, f func(string) bool) []string {
    vsf := make([]string, 0)
    for _, v := range vs {
        if f(v ) {
            vsf = append(vsf, v)
...
func main() {
    ...
    fmt.Println(Filter(strs, func(v string) bool {
        return strings.Contains(v, "pear")
    }))
    ...
    //of the elements of the slice. Unlike an array type, a slice
    //type has no specified length.
        
    results := []string{""}
    var item = "apple"
    
    //results = Filter(strs, func(v string) bool {
    //    return strings.Contains(v, "apple")
    //})
   results = MyFilter(strs, func(v string) bool {
